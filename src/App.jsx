...
export default function QigongApp() {
  ...
  useEffect(() => {
    if (!cargado) return;
    try {
      localStorage.setItem("progreso-qigong", JSON.stringify({ completadas, premium, avisoAceptado, introVista }));
    } catch (e) {}
  }, [completadas, premium, avisoAceptado, introVista, cargado]);
    ...
    <div className="min-h-screen w-full flex justify-center" style={{ background: "#0F1B2D" }}>
      ...
      >
        {!cargado ? (
          ...
          <Beneficios onContinuar={() => setIntroVista(true)} />
        ) : (
          <>
            {vista === "modulos" && (
              <Modulos
                modulos={MODULOS}
                proximos={PROXIMOS_MODULOS}
...
// ---------------------------------------------------------------------
function Beneficios({ onContinuar }) {
  const beneficios = [
    {
      icono: <Flame size={20} />,
      titulo: "Más energía y vitalidad",
      texto: "Activa la circulación del qi y despierta una energía sostenida durante todo el día, sin altibajos.",
    },
    {
      icono: <Wind size={20} />,
      titulo: "Calma el sistema nervioso",
      texto: "La respiración consciente y el movimiento lento regulan el estrés y devuelven la sensación de centro.",
    },
    {
      icono: <Sparkles size={20} />,
      titulo: "Cuerpo más flexible y presente",
      texto: "Mejora el equilibrio, la movilidad articular y la conciencia corporal, incluso practicando poco tiempo.",
    },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center px-7 py-10" style={{ background: "#F6F1E4" }}>
      ...
      <p className="text-[13px] text-center mb-7" style={{ color: "#5C6B7A", fontFamily: "system-ui" }}>
        Una práctica milenaria, con efectos que se sienten desde la primera sesión.
      </p>
      ...
      <div className="rounded-2xl p-4 mb-7" style={{ background: "#1B2A4A" }}>
        <p className="text-[14px] text-center leading-relaxed" style={{ color: "#F6F1E4", fontFamily: "system-ui" }}>
          Con solo <strong style={{ color: "#C9912A" }}>15-20 minutos al día</strong>, sostenidos en el tiempo, tu energía, tu cuerpo y tu calma interior mejoran muchísimo.
        </p>
      </div>
...
// ---------------------------------------------------------------------
function Modulos({ modulos, proximos, completadas, premium, onAbrir, onTogglePremium, onReiniciar }) {
  return (
    <div className="pb-12">
      <div
